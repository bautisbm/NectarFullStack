...
SELECT DISTINCT post.sch_year, sch_name, SUM (ps_enrollment_cnt) as total_enrollment_cnt
FROM school AS sch
	JOIN postsec_enroll AS post ON sch.div_num = post.div_num AND sch.sch_num = post.sch_num
	JOIN division AS div ON sch.div_num = div.div_num
WHERE sch.div_num = $1
...
GROUP BY sch_year, sch_name, sch.div_num
